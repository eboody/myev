...
fn print_bits(dev: &Device) {
    ...
    for ev_type in EventTypeIterator::new() {
        ...
        match ev_type {
            EventType::EV_KEY | EventType::EV_REL | EventType::EV_ABS | EventType::EV_LED => {
                print_code_bits(dev, &ev_type)
            }
            _ => (),
        }
    }
...
fn main() {
    ...
    let physical_device = u_d.set_file(file).unwrap();

    println!(
        "Input device ID: bus 0x{:x} vendor 0x{:x} product 0x{:x}",
        physical_device.bustype(),
        physical_device.vendor_id(),
        physical_device.product_id()
    );
    println!("Evdev version: {:x}", physical_device.driver_version());
    println!(
        "Input device name: \"{}\"",
        physical_device.name().unwrap_or("")
    );
    println!("Phys location: {}", physical_device.phys().unwrap_or(""));
    println!("Uniq identifier: {}", physical_device.uniq().unwrap_or(""));

    print_bits(&physical_device);
    print_props(&physical_device);

    let virtual_device = Device::new_from_path(Path::new(path)).unwrap();

    for ev_type in EventTypeIterator::new() {
        if physical_device.has(ev_type) {
            let _ = virtual_device.enable_event_type(&ev_type);
        }

        for code in EventCodeIterator::new(&ev_type) {
            if !physical_device.has(code) {
                continue;
            }
            let _ = virtual_device.enable_event_code(&code, None);
        }
    }

    let mut a: io::Result<(ReadStatus, InputEvent)>;
    loop {
        a = virtual_device.next_event(ReadFlag::NORMAL);
        if a.is_ok() {
            ...
            match result.0 {
                ReadStatus::Sync => {
                    ...
                    while result.0 == ReadStatus::Sync {
                        ...
                        a = virtual_device.next_event(ReadFlag::SYNC);
                        if a.is_ok() {
                            result = a.ok().unwrap();
                        } else {
                            break;
